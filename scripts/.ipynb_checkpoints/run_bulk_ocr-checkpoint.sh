...
mkdir -p "$PID_DIR" "$DATA_ROOT"
SIDECAR_PID="$PID_DIR/sidecar.pid"
BULK_PID="$PID_DIR/bulk.pid"
MONITOR_PID="$PID_DIR/monitor.pid"

SIDECAR_LOG="$DATA_ROOT/sidecar.log"
BULK_LOG="$DATA_ROOT/bulk_ocr.log"
MONITOR_LOG="$DATA_ROOT/ocr_monitor.log"
...
apply_settings() {
  curl -fsS -X PUT "$BASE_URL/settings" \
    ...
    -d "{\"ocr_page_concurrency\":${PAGE},\"ocr_document_concurrency\":${DOC},\"ocr_api_concurrency\":${API},\"ocr_worker_processes\":${WORKERS},\"vision_pdf_dpi\":300,\"llm_provider\":\"dashscope\"}" \
    >/dev/null
  echo "settings applied: page=${PAGE} doc=${DOC} api=${API} workers=${WORKERS} dpi=300 dashscope"
}
